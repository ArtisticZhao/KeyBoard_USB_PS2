...
// ------------ 自定义的makesure 函数, 保证按键被正确按下
void PS2dev::keyboard_press_makesure(unsigned char code)
{
  int ret = 1;
  while (ret != 0){
    ret = keyboard_press(code);
  }
}

void PS2dev::keyboard_release_makesure(unsigned char code)
{
  int ret = 1;
  while (ret != 0){
    ret = keyboard_release(code);
  }
}

void PS2dev::keyboard_press_special_makesure(unsigned char code)
{
  int ret = 1;
  while (ret != 0){
    ret = keyboard_press_special(code);
  }
}

void PS2dev::keyboard_release_special_makesure(unsigned char code)
{
  int ret = 1;
  while (ret != 0){
    ret = keyboard_release_special(code);
  }
}