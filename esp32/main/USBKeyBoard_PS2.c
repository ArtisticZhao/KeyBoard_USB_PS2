            ...
            switch(event.type) {
                ...
                case UART_DATA:
                    ...
                    for (size_t i = 0; i < event.size; ++i) {
                        sprintf((char*)(rxdata+i*3), "%02X ", dtmp[i]);
                    }
                    rxdata[event.size*3] = '\0';
                    ESP_LOGI(TAG, "[DATA EVT]:>%s", rxdata);
                    break;
...
void app_main(void)
{
    ...
    uart_enable_pattern_det_baud_intr(EX_UART_NUM, '0xAB', PATTERN_CHR_NUM, 9, 0, 0);
